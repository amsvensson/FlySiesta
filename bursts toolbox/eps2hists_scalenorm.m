function [histogram,survival_histogram,x]=eps2hists_scalenorm(Eps,scale,mintime,binsize)
% Calculate scaled probability density distribution in HISTOGRAM and survival
% distribution in SURVIVAL_HISTOGRAM, from Episodes EPS, with scale SCALE. Optional input 
...
if ~isempty(Eps)
  ...
  if ~iscell(Eps)
    if all(size(Eps)>1)
      error('bursts:eps2hists_scalenorm:InputDimensions','Valid input are 1 dimensional cell array or numerical vector.')
    elseif all(size(Eps)==1)
      % Input is scalar.
      if Eps >= mintime
        Eps={Eps};
      else
        error('bursts:eps2hists_scalenorm:ScalarInput','Valid input are 1 dimensional cell array or numerical vector.')
      end
    else
  ...
  if nargin<2 || isempty(scale)
    scale=ones(size(Eps));
  elseif ~all(size(scale)==size(Eps))
    error('bursts:eps2hists_scalenorm:DimensionMismatch','Eps and Scale must have the same dimensions.')
  end
  
  maxEps=cellfun(@max,Eps);
  x = [mintime : binsize : max(maxEps)]/max(scale);
